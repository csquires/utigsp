...
DagSetting = namedtuple('DagSetting', 'nneighbors')
SampleSetting = namedtuple('SampleSetting', ['nsamples', 'ntargets', 'nsettings'])
AlgSetting = namedtuple('AlgSetting', ['nruns', 'depth', 'alpha', 'alpha_invariant'])


def tqdm_dict_items(d):
    return tqdm(d.items(), total=len(d))


def _sample_setting2str(nsamples, ntargets, nsettings):
    return 'nsamples=%d,ntargets=%s,nsettings=%s' % (nsamples, ntargets, nsettings)
...
class DagConfig:
    ...
    def setting_folders(self):
        ...
        return {
            DagSetting(nneighbors=nneighbors): os.path.join(base_folder, 'nneighbors=%.2f' % nneighbors)
            for nneighbors in self.nneighbors_list
        }
    ...
    def _save_samples(self, verbose=True):
        ...
        for dag_setting, graphs in self.dag_config.load_graphs().items():  # DO THIS FIRST SO WE LOAD EACH GRAPH ONCE
            ...
            for g, dag_folder in zip(graphs, self.dag_config.setting2dag_folders[dag_setting]):
                ...
                for nsamples, ntargets, nsettings in itr.product(self.nsamples_list, self.ntargets_list, self.nsettings_list):
                    # === RANDOMLY PICK INTERVENTION NODES AND WHICH ARE KNOWN
                    all_iv_nodes_list = [frozenset(random.sample(nodes_list, sum(ntargets))) for _ in range(nsettings)]
                    known_iv_nodes_list = [frozenset(random.sample(all_iv_nodes, ntargets[0])) for all_iv_nodes in all_iv_nodes_list]

                    # === CREATE FOLDER TO SAVE SAMPLES
                    sample_folder = os.path.join(dag_folder, 'samples', _sample_setting2str(nsamples, ntargets, nsettings))
                    iv_sample_folder = os.path.join(sample_folder, 'interventional')
                    os.makedirs(iv_sample_folder)
                    ...
                    for all_iv_nodes, known_iv_nodes in zip(all_iv_nodes_list, known_iv_nodes_list):
                        samples = g.sample_interventional({iv_node: self.intervention for iv_node in all_iv_nodes}, nsamples)
                    ...
                    sample_settings2samples_dict[SampleSetting(nsamples=nsamples, ntargets=ntargets, nsettings=nsettings)] = samples_dict
...
def _run_alg_graph(tup):
    ...
    for ss in sample_settings:
        # === LOAD SAMPLES INTO DICTIONARY
        sample_folder = os.path.join(dag_folder, 'samples', _sample_setting2str(ss.nsamples, ss.ntargets, ss.nsettings))
        iv_sample_folder = os.path.join(sample_folder, 'interventional')
        samples_dict = {frozenset(): np.loadtxt(os.path.join(sample_folder, 'observational.txt'))}
        for fn in os.listdir(iv_sample_folder):
            known_ivs = frozenset(map(int, fn.split(';')[0].split('=')[1].split(',')))
...
if __name__ == '__main__':
    test_dag_config = DagConfig(dataset_name='test', nnodes=8, nneighbors_list=[1], ngraphs=100)
