from dataset_configs.fig1.configs import dag_config, sample_config, alg_config, DATASET_NAME
...
shd_array_igsp_r = shd_array_dict['igsp_r_multi'].sel(nsettings=nsettings)
...
imec_array_igsp_r = imec_array_dict['igsp_r_multi'].sel(nsettings=nsettings)
...
mean_shd_igsp_r = shd_array_igsp_r.mean(dim='dag')
percent_consistent_igsp_r = (shd_array_igsp_r == 0).mean(dim='dag')
...
percent_correct_imec_igsp_r = imec_array_igsp_r.mean(dim='dag')
...
for ntargets, ls in zip(ntargets_list, LINESTYLES):
    ...
    plt.plot(mean_shd_igsp_r.nsamples, mean_shd_igsp_r.sel(ntargets=ntargets).squeeze(), color=ALGS2COLORS['igsp_r'], linestyle=ls)
...
for ntargets, ls in zip(ntargets_list, LINESTYLES):
    ...
    plt.plot(percent_consistent_igsp_r.nsamples, percent_consistent_igsp_r.sel(ntargets=ntargets).squeeze(), color=ALGS2COLORS['igsp_r'], linestyle=ls)
plt.legend(handles=handles)
# plt.xticks(mean_shd_igsp.nsamples)
plt.xlabel('Number of samples')
...
for ntargets, ls in zip(ntargets_list, LINESTYLES):
    ...
    plt.plot(percent_correct_imec_igsp_r.nsamples, percent_correct_imec_igsp_r.sel(ntargets=ntargets).squeeze(), color=ALGS2COLORS['igsp_r'], linestyle=ls)
plt.legend(handles=handles)
# plt.xticks(mean_shd_igsp.nsamples)
plt.xlabel('Number of samples')
