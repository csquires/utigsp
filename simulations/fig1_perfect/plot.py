...
if JCIGSP:
    ...
    learned_iv_array_jcigsp = utils.empty_array(coords)
    missing_iv_array_jcigsp = utils.empty_array(coords)
    added_iv_array_jcigsp = utils.empty_array(coords)
...
for nsamples, nsettings, (num_known, num_unknown) in itr.product(nsamples_list, nsettings_list, ntargets_list):
    ...
    if UTIGSP:
        ...
        if os.path.exists(os.path.join(utigsp_results_folder)):
            ...
            learned_iv_array_utigsp.loc[loc] = np.mean(np.load(os.path.join(utigsp_results_folder, 'diff_interventions.npy')), axis=1)
    ...
    if JCIGSP:
        ...
        if os.path.exists(os.path.join(jcigsp_results_folder)):
            ...
            learned_iv_array_jcigsp.loc[loc] = np.mean(np.load(os.path.join(jcigsp_results_folder, 'diff_interventions.npy')), axis=1)
            missing_iv_array_jcigsp.loc[loc] = np.mean(np.load(os.path.join(jcigsp_results_folder, 'missing_interventions.npy')), axis=1)
            added_iv_array_jcigsp.loc[loc] = np.mean(np.load(os.path.join(jcigsp_results_folder, 'added_interventions.npy')), axis=1)
...
plt.tight_layout()
plt.savefig(os.path.join(PLT_FOLDER, f'shd-{NAME}.png'))
...
axes[0].legend(handles=alg_handles, loc='upper center')
plt.tight_layout()
plt.savefig(os.path.join(PLT_FOLDER, f'shd-icpdag-{NAME}{"_gies" if GIES else ""}.png'))
...
plt.tight_layout()
plt.savefig(os.path.join(PLT_FOLDER, f'correct-imec-{NAME}.png'))
...
axes[-1].legend(handles=alg_handles, loc='upper center')
plt.tight_layout()
plt.savefig(os.path.join(PLT_FOLDER, f'consistent-icpdag-{NAME}.png'))
...
for num_unknown, marker in zip([0, 1, 2, 3], MARKERS):
    axes[0].plot(nsamples_list, missing_iv_array_utigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='r', marker=marker)
    axes[1].plot(nsamples_list, added_iv_array_utigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='b', marker=marker)
...
plt.tight_layout()
plt.savefig(os.path.join(PLT_FOLDER, f'recovered-targets-{NAME}.png'))

plt.clf()
for num_unknown, marker in zip([0, 1, 2, 3], MARKERS):
    plt.plot(nsamples_list, missing_iv_array_utigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='b', marker=marker)
    plt.plot(nsamples_list, missing_iv_array_jcigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='r', marker=marker)
...
plt.savefig(os.path.join(PLT_FOLDER, f'missing-targets-{NAME}.png'))

plt.clf()
for num_unknown, marker in zip([0, 1, 2, 3], MARKERS):
    plt.plot(nsamples_list, added_iv_array_utigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='b', marker=marker)
    plt.plot(nsamples_list, added_iv_array_jcigsp.mean(dim='dag').sel(num_unknown=num_unknown), color='r', marker=marker)
plt.xticks(nsamples_list)
plt.xlabel('Number of samples')
...
plt.savefig(os.path.join(PLT_FOLDER, f'added-targets-{NAME}.png'))
